class PaymentScreen extends Component {
  ...
  componentDidMount() {
    ...
    const requestData = {
      ...
      last_name: user?.name,
      trackid: `${user?.id}-${new Date().getTime()}`,
      udf1: user?.id,
      udf2: responseUrl,
      udf3: language,
    };
  ...
  returnResponse = (data) => {
    console.log(
      `debug ~ file: PaymentScreen.tsx ~ line 92 ~ PaymentScreen ~ data`,
      data
    );
    const { navigation } = this.props;
    navigation.replace("RESPONSE_SCREEN", { response: data.data });
  };
        ...
        .then((ip) => {
          ...
          axios
            ...
            .catch(function (error) {
              console.log(
                `debug ~ file: PaymentScreen.tsx ~ line 385 ~ PaymentScreen ~ .then ~ error`,
                { ...error }
              );
              console.log(error);
